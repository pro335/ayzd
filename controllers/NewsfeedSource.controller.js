...
exports.createNewsfeedSource = async (req, res) =>  {
    ...
    try {
        ...
        project = await Project.findById(req.body.project).populate({path: "category", select: ["_id", "name"] }).populate({path: "chain", select: ["_id", "name"] }).populate({path: "main_image", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "secondary_image", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "newsfeedSource_list", select: ["_id", "link", "project", "keyword_list"]}).populate({path: "media_list", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "guide_list", populate: [{path: "media_video", select: ['name', 'url']}, {path: "media_image", select: ['name', 'url']}]}).populate({path: "member_list", populate: {path: "avatar"}, select: ["_id", "name", "avatar", "position", "facebook_link", "twitter_link", "dribbble_link", "instagram_link", "medium_link"] }).populate({path: "similar_list", populate: [{path: "main_image"}, {path: "secondary_image"}, {path: "category"}]});
    } catch(err) {
        console.log(err)
    ...
    NewsfeedSource.findByIdAndRemove(id, {
        ...
        if (err) {
            ...
            if (newsfeedSource !== null) {
                let project = await Project.findById(newsfeedSource.project).populate({path: "category", select: ["_id", "name"] }).populate({path: "chain", select: ["_id", "name"] }).populate({path: "main_image", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "secondary_image", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "newsfeedSource_list", select: ["_id", "link", "project", "keyword_list"]}).populate({path: "media_list", select: [ "_id", "name", "url", "type", "relation" ] }).populate({path: "guide_list", populate: [{path: "media_video", select: ['name', 'url']}, {path: "media_image", select: ['name', 'url']}]}).populate({path: "member_list", populate: {path: "avatar"}, select: ["_id", "name", "avatar", "position", "facebook_link", "twitter_link", "dribbble_link", "instagram_link", "medium_link"] }).populate({path: "similar_list", populate: [{path: "main_image"}, {path: "secondary_image"}, {path: "category"}]});
