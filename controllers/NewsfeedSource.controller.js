...
exports.createNewsfeedSource = async (req, res) =>  {
    ...
    try {
        ...
        project = await Project.findById(req.body.project).populate("category").populate("chain").populate("main_image").populate("secondary_image").populate("newsfeedSource_list").populate("media_list").populate({path: "guide_list", populate: [{path: "media_video", select: ['name', 'url']}, {path: "media_image", select: ['name', 'url']}]}).populate({path: "member_list", populate: {path: "avatar"}}).populate({path: "similar_list", populate: [{path: "main_image"}, {path: "secondary_image"}, {path: "category"}]});
    } catch(err) {
        console.log(err)
    ...
    NewsfeedSource.findByIdAndRemove(id, {
        ...
        if (err) {
            ...
            if (newsfeedSource !== null) {
                let project = await Project.findById(newsfeedSource.project).populate("category").populate("chain").populate("main_image").populate("secondary_image").populate("newsfeedSource_list").populate("media_list").populate({path: "guide_list", populate: [{path: "media_video", select: ['name', 'url']}, {path: "media_image", select: ['name', 'url']}]}).populate({path: "member_list", populate: {path: "avatar"}}).populate({path: "similar_list", populate: [{path: "main_image"}, {path: "secondary_image"}, {path: "category"}]});
