  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        let date_after_some_days = moment(new Date())
          .add(i, "days")
          .format("MMMM D");
      ...
      temp_upcomings.map((item) => {
        ...
        let foundIndex = temp_upcoming_date_list.findIndex(
          (x) => x.date === new_date
        );
        if (foundIndex !== -1) {
      ...
      if (isValid(temp_previous_upcoming_date_list)) {
        temp_previous_upcoming_show_list = temp_previous_upcomings.filter(
          function (item) {
            return (
              moment(item.upcoming_date).format("MMMM D") ===
              temp_previous_upcoming_date_list[0].date
            );
          }
        );
      }
